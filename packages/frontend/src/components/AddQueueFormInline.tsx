...
function mapServiceOptions(services?: Service[]): Array<{ value: string; label: string }> {
...
const serviceOptions = useMemo(() => {
  const dynamic = mapServiceOptions(services);
  if (dynamic.length > 0) {
    return dynamic;
  }
  return SERVICE_TYPE_OPTIONS.map((option) => ({
    value: option.value,
    label: option.label,
  }));
}, [services]);
  ...
  useEffect(() => {
    if (!formData.serviceType && serviceOptions.length > 0) {
      setFormData((prev) => ({
        ...prev,
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      queueApi
        ...
        .catch((error) => console.warn("Failed to save queue form preference", error));
      ...
      setFormData({
        ...
        patientName: "",
        patientId: undefined,
        serviceType: serviceOptions[0]?.value || formData.serviceType,
        priority: formData.priority,
        assignedVetId: "NONE",
        hasScheduledAppointment: false,
        scheduledAt: "",
  ...
  return (
    <>
      <form
        ...
        className={
          inline
            ? "bg-card border rounded-lg p-5 space-y-6"
            : "space-y-6 border rounded-lg p-6 bg-background"
        }
      >
        ...
        {currentStepIndex === 1 && (
          <div className="space-y-5">
            <div className="flex items-center justify-between">
              <Button type="button" variant="ghost" onClick={() => setCurrentStepIndex(0)}>
                Voltar
              </Button>
      ...
      <Dialog open={showTutorDialog} onOpenChange={setShowTutorDialog}>
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label htmlFor="quickTutorNameInline">Nome *</Label>
              <Input
                id="quickTutorNameInline"
                value={newTutor.name}
                onChange={(event) =>
            ...
            <div className="space-y-2">
              <Label htmlFor="quickTutorPhoneInline">Telefone</Label>
              <Input
                id="quickTutorPhoneInline"
      ...
      <Dialog open={showPatientDialog} onOpenChange={setShowPatientDialog}>
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label htmlFor="quickPatientNameInline">Nome *</Label>
              <Input
                id="quickPatientNameInline"
            ...
            <div className="space-y-2">
              <Label htmlFor="quickPatientSpeciesInline">Espécie</Label>
              <Input
                id="quickPatientSpeciesInline"
            ...
            <div className="space-y-2">
              <Label htmlFor="quickPatientNotesInline">Observações</Label>
              <Input
                id="quickPatientNotesInline"
