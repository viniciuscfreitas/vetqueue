...
import { auditApi, userApi, AuditLog, ModuleKey, User } from "@/lib/api";
...
const moduleLabels: Record<ModuleKey, string> = {
  [ModuleKey.QUEUE]: "Fila",
  [ModuleKey.PATIENTS]: "Pacientes",
  [ModuleKey.TUTORS]: "Tutores",
  [ModuleKey.FINANCIAL]: "Financeiro",
  [ModuleKey.ADMIN_USERS]: "Admin · Usuários",
  [ModuleKey.ADMIN_ROOMS]: "Admin · Salas",
  [ModuleKey.ADMIN_SERVICES]: "Admin · Serviços",
  [ModuleKey.PERMISSIONS]: "Permissões",
  [ModuleKey.REPORTS]: "Relatórios",
  [ModuleKey.AUDIT]: "Auditoria",
};

const moduleOptions = Object.entries(moduleLabels).map(([value, label]) => ({
  value: value as ModuleKey,
  label,
}));
...
export function AuditView({ authLoading }: AuditViewProps) {
    ...
    queryFn: () =>
      auditApi
        .getLogs({
          ...
          module: filters.module === "__ALL__" ? undefined : (filters.module as ModuleKey),
          page,
          limit: 20,
        })
        .then((res) => res.data),
  ...
  const getEntityTypeLabel = (entityType: string) => {
    ...
    return entityTypeConfig[normalized] || normalized;
  };

  const getModuleLabel = (module?: ModuleKey) => {
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Visualize todas as ações registradas pelos módulos do sistema
          </p>
        </div>
        ...
        <Card>
          <CardContent className="pt-4">
            <div className="flex flex-wrap gap-3 items-end">
              ...
              <div className="min-w-0">
                <Label htmlFor="auditModule" className="text-xs mb-1 block text-muted-foreground">
                  Módulo
                </Label>
                <Select
              ...
              <div className="min-w-0">
                <Label htmlFor="auditEntityType" className="text-xs mb-1 block text-muted-foreground">
                  Entidade
                </Label>
                <Select
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="__ALL__">Todas</SelectItem>
                    {Object.entries(entityTypeConfig).map(([value, label]) => (
