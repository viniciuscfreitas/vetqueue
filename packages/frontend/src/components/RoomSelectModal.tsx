...
import { roomApi, queueApi, Role, ModuleKey } from "@/lib/api";
import {
  Select,
  SelectContent,
  SelectItem,
...
export function RoomSelectModal({ open, onSelect, onCancel }: RoomSelectModalProps) {
  ...
  const isReceptionFlow = canManageQueue && !isVet;
  ...
  const { data: occupations = {} } = useQuery({
    ...
    enabled: (isVet || isReceptionFlow) && open,
  });

  const roomsWithVets = Object.keys(occupations);
  const availableRooms = isReceptionFlow
  ...
  const isSelectedDisabled = isReceptionFlow && !selectedOccupation;
  ...
  const handleConfirm = useCallback(() => {
    if (selectedRoomId) {
      ...
      if (isReceptionFlow && !occupations[selectedRoomId]) {
        return;
      }
      onSelect(selectedRoomId);
    }
  }, [selectedRoomId, isVet, isReceptionFlow, occupations, onSelect]);
      ...
      <div className="bg-background border rounded-lg shadow-xl p-6 w-full max-w-md">
        ...
        <div className="space-y-4">
          <div>
            ...
            {selectedRoom && selectedOccupation && isReceptionFlow && (
              <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-md flex items-center gap-2">
                <CheckCircle2 className="h-4 w-4 text-green-600 flex-shrink-0" />
                <p className="text-sm font-medium text-green-900">
            ...
            {isReceptionFlow && availableRooms.length === 0 && (
              <div
